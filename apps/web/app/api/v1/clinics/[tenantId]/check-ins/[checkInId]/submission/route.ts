import { submitCheckIn } from "@/modules/check-ins/service";
...
export async function POST(request:Request,{params}:{params:Promise<{tenantId:string;checkInId:string}>}){
  ...
  try{const p=await params;return json(await submitCheckIn(p.tenantId,p.checkInId,await boundedJson(request,12000)),201);}catch(e){return apiError(e);}
}
