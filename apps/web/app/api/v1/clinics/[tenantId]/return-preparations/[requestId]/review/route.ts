import { reviewReturnPreparation } from "@/modules/return-preparation/service";
...
export async function POST(request: Request, { params }: { params: Promise<{ tenantId: string; requestId: string }> }) {
  ...
  try {
    ...
    return json(await reviewReturnPreparation(value.tenantId, value.requestId, await boundedJson(request, 5000)), 201);
  } catch (error) {
