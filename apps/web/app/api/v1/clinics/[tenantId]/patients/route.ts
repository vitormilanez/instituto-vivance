...
export async function GET(request: Request, { params }: Context) {
  try { return json(await listPatients((await params).tenantId, pageNumber(new URL(request.url).searchParams.get('page')))); }
...
export async function POST(request: Request, { params }: Context) {
  ...
  try {
    ...
    while (true) {
      ...
      if (size > 4096) { await reader.cancel(); return json({ error: 'Cadastro muito grande.' }, 413); }
      chunks.push(value);
    }
    const body = JSON.parse(Buffer.concat(chunks).toString('utf8'));
    return json({ patient: await createPatient((await params).tenantId, body) }, 201);
  } catch (error) { return apiError(error); }
}
