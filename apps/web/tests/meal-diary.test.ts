...
test("o diário alimentar entra na jornada existente sem criar uma nona ação rápida", () => {
  ...
  assert.match(homeLogs, /\/refeicao`/);
  // O diário continua em "Diário"; o registro rápido é a tarefa "refeicao".
  assert.match(patientPage, /\["diario", "refeicao"\]\.includes\(slug\) \? patientMeals\(tenantId\)/);
