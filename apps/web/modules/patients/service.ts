...
export async function listPatients(id: string, page = 1) {
  ...
  const { data, error, count } = await context.client.from('patients')
    .select('id, display_name, birth_date, created_at', { count: 'exact' }).eq('tenant_id', id)
    .order('display_name').order('id').range((page - 1) * 25, page * 25 - 1);
...
export async function createPatient(id: string, input: unknown) {
  ...
  const { data, error } = await client.from('patients').insert({ tenant_id: id, ...values })
    .select('id, display_name, birth_date, created_at').single();
