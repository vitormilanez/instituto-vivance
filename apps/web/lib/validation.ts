...
export function patientInput(value: unknown) {
  if (!value || typeof value !== 'object' || Array.isArray(value)) throw new InputError('Cadastro inválido.');
  const body = value as Record<string, unknown>;
  if (Object.keys(body).some(key => !['display_name', 'birth_date'].includes(key))) {
    throw new InputError('O cadastro contém campos não permitidos.');
  }
  if (typeof body.display_name !== 'string') throw new InputError('Informe o nome do paciente.');
  ...
  const birth = body.birth_date === '' || body.birth_date === undefined ? null : body.birth_date;
  if (birth !== null && (typeof birth !== 'string' || !/^\d{4}-\d{2}-\d{2}$/.test(birth)
    ...
    || birth < '1900-01-01' || birth > new Date().toISOString().slice(0, 10))) {
...
export function pageNumber(value: string | null | undefined): number {
  if (!value) return 1;
  if (!/^\d+$/.test(value) || Number(value) < 1 || Number(value) > 10000) throw new InputError('Página inválida.');
  return Number(value);
}
