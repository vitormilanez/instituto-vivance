...
Quatro testes existentes de publicação/isolamento também passaram, sem ampliar a suíte. Capturas desktop 1600px/celular 390px e revisão visual independente sem correções materiais adicionais. Dados sintéticos desta rodada serão removidos após a prova; não são conteúdo do produto. O 4C foi concluído localmente abaixo; próximo incremento local: 4D. A composição segue o protótipo com conteúdo disponível, sem prometer reprodução de funções ainda não implementadas.

### Slice 4C — check-in manual validado localmente
...
- Versão funcional local `32f8ae7`, sem push, CI, build completo ou novo deployment. A Preview protegida continua no 4B (`98942c7`) até o fechamento do lote visual + 4C + 4D. Um paciente `QA 4C · Paciente sintético` e seu check-in permanecem somente no Supabase de desenvolvimento para a validação do 4D; devem ser removidos por IDs conferidos após essa prova. Registros anteriores foram preservados.
- Limites: uma solicitação manual aceita uma pergunta por vez e medida genérica opcional; pré-consulta aqui é o mesmo mecanismo manual, sem questionário clínico configurável. A fila inclui histórico revisado e, acima de 20 itens, um link do painel Hoje pode exigir navegar para outra página; tratar no 4D antes de escalar. A interface ainda usa autoria genérica e contato da clínica não configurado. Sem notificações, fotos, áudio, cálculo longitudinal ou uso com dados reais. A tarefa maior continua incompleta porque o 4D e o Gate P ainda não foram validados.
...
- Base publicada conferida: commit `98942c7`, com 3B–3D e 4A–4B encerrados. Incremento visual local `de96847` e 4C local `32f8ae7`, ainda sem push/deploy. Evidências abaixo.
...
Asana: a tarefa `Slice 3B–3D — Base clínica e operação de atendimento` (`1218384571286751`) teve os seis BDDs e as evidências atualizados, foi marcada como concluída e movida para `Done`. A tarefa `Slice 4A–4D — Planos de cuidado e acompanhamento` (`1218384336844795`) permanece aberta em Development: 4A–4C estão validados e o 4D continua pendente.
...
**Próxima implementação recomendada: 4D — Acompanhamento longitudinal.** Compor linha do tempo e medidas somente a partir dos registros persistidos no 4C e das publicações existentes, mantendo separados conteúdo do paciente e revisão interna.
...
| **4C — Check-ins, diário e pré-consulta — validado localmente** | Paciente envia relato/medidas e equipe vinculada registra revisão | Entregue localmente: autoria/data/origem, conteúdo original, revisão interna, fila real, isolamento, idempotência e auditoria; sem diagnóstico, urgência ou alteração automática do plano. Preview aguarda o fechamento do lote |
...
Próximo: **4D — Acompanhamento longitudinal**. 4A e 4B estão encerrados em Preview protegida no código `98942c7`; o incremento visual `de96847` e o 4C `32f8ae7` estão validados e commitados somente localmente. O 4C preserva relato/autoria/data/origem, oferece medida genérica opcional, cria pendência real para a equipe e mantém a revisão interna fora do portal do paciente; não diagnostica, classifica urgência ou altera plano. Reutilizar esses dados persistidos no 4D para compor linha do tempo e medidas neutras, sem inventar indicadores. Após o 4D, remover a fixture `QA 4C`, executar checks completos/build e então publicar uma única Preview protegida do lote. Não promover Production nem marcar a tarefa maior como concluída antes de validar o 4D e os gates aplicáveis.
