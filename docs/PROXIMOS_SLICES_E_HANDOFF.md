...
### Slice 5D — base privada de processamento validada no desenvolvimento

- `processing_jobs` é a fila durável para futuras tarefas autorizadas, com estados pendente → processando → concluído/falhou, uma reserva de cinco minutos, no máximo cinco tentativas e repetição controlada. A chave idempotente é única por clínica e não pode ser reutilizada para outro paciente ou tipo de tarefa.
- A tabela guarda somente contexto autorizado, estado, horários, tentativas e código genérico de falha. Não armazena áudio, texto clínico, prompt, resposta de fornecedor, segredo ou erro bruto. A auditoria não copia a chave de origem.
...
| **5D — Base de processamento — validada no desenvolvimento** | Tarefa autorizada tem estado, idempotência, reserva, retentativa e falha controlada | RLS por vínculo, leitura clínica e executor exclusivo de servidor; sem payload clínico na fila, worker, fornecedor, áudio ou IA. Preview pendente desta rodada; saúde real depende do Gate P |

### Fase D — áudio e processamento assíncrono

| Slice | Resultado demonstrável | Aceite essencial / dependência |
| --- | --- | --- |
| **5D — Base de processamento — validada no desenvolvimento** | Tarefa percorre pendente → processando → concluída/falhou, com repetição controlada | Contexto de clínica/paciente validado no executor; idempotência, limites, timeout, retentativas e tratamento de falha definitiva. Sem payload clínico nos logs. Não há worker, fornecedor ou custo ativo |
...
Os Slices **5B** e **5C** estão completos no checkout, no Supabase de desenvolvimento e na Preview protegida: conversa direta paciente–médico e avisos internos genéricos, sem canal externo ou promessa de cobertura. O Slice **5D** está aplicado no Supabase de desenvolvimento e validado localmente: base privada de processamento com RLS, idempotência, reserva e falha controlada, mas sem worker ou processamento real. A próxima fatia recomendada é **5E — Captura privada de áudio**; antes dela é necessária uma decisão explícita sobre consentimento, retenção, fornecedor que eventualmente receberá áudio, custo e revisão humana. O Gate P continua obrigatório para qualquer dado de saúde real.
