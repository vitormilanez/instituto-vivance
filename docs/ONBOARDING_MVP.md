...
Antes de ativar em ambiente conectado: reconciliar o histórico remoto de migrations, aplicar a nova migration e publicar `invite-patient`/`claim-patient-invitation`; configurar `PATIENT_INVITE_REDIRECT_URL` como URL HTTPS exata de `/primeiro-acesso` e incluí-la nas URLs permitidas do Auth. A função pública `claim-patient-invitation` precisa ser publicada com `--no-verify-jwt`, pois a pessoa ainda não tem sessão; seu acesso é limitado pelo token opaco de uso único. A função `invite-patient` mantém verificação de JWT e revalida identidade/papel. Não desabilitar JWT das outras funções.

Validar então, com contas descartáveis no ambiente autorizado, entrega do email, primeiro acesso, aceite, retomada após novo login, foto/exames e leitura pelo médico. Nenhuma configuração, migration, função, mensagem, Preview ou Production remota foi alterada nesta entrega. Não há comprovante de entrega de email, recuperação automática de link perdido, antivírus, OCR, IA ou áudio.
